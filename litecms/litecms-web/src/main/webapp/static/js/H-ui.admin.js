...
function initValidate(validDesc){
	var _gvd=validDesc;
	if(_gvd){
		for(var i=0; i<_gvd.length; i++){
			var _input=$("[name='"+_gvd[i].fieldName+"']");
			if(!_input.is(":hidden") && !_input.is(":checkbox") && !_input.is(":radio") && !_input.is(".ignore")){
				var vt=validTypes[_gvd[i].validType];
				if(vt=="required" || vt=="creditcard" || vt=="email"){
					_input.addClass(vt).attr("data-msg-"+vt,_gvd[i].message);
				}else{
					var _vtVal=_gvd[i].attrs[vt];
					if(_vtVal || vt=="min" || vt=="max"){
						_vtVal=_vtVal||_gvd[i].attrs["value"];
						_input.attr(vt,_vtVal||"").
							attr("data-msg-"+vt,_gvd[i].message);
					}else{
						if(vt=="rangelength"){
							_input.attr("rangelength",[_gvd[i].attrs["min"],_gvd[i].attrs["max"]]).
								attr("data-msg-"+vt,_gvd[i].message);
						}else{
							for(var _a in _gvd[i].attrs){
								_input.attr(_a,_gvd[i].attrs[_a]).
									attr("data-msg-"+_a,_gvd[i].message);
							}
						}
					}
				}
